...
export default function AIChatPanel({
  ...
  const { isChatOpen, closeChat, provider, setProvider, getUserApiKey } =
    useAI();
  ...
  useEffect(() => {
    ...
    for (const rawPart of rawParts) {
      ...
      if (state === "output-available" || state === "output-streaming")
        continue;
      ...
      try {
        if (toolName === "read_file") {
          ...
          if (!path || typeof path !== "string") {
            ...
            result =
              file && "content" in file && file.content !== undefined
                ? file.content
          ...
          if (!path || typeof path !== "string") {
            ...
            const existingFile = findFileByPath(templateData.items, path);
            const oldContent =
              existingFile &&
              "content" in existingFile &&
              existingFile.content !== undefined
            ...
            setPendingChanges({
              toolCallId,
              toolName,
              changes: [{ path, oldContent, newContent: content as string }],
            });
            setIsReviewPending(true);
            result = `Changes pending review: edit_file ${path}`;
          }
        } else if (toolName === "edit_multiple_files") {
          const { changes } = args as {
          ...
          if (!changes || !Array.isArray(changes) || changes.length === 0) {
            ...
            // Get current content for each file
            const mappedChanges: Array<{
              path: string;
              oldContent: string;
              newContent: string;
            }> = changes.map((change) => {
            ...
            setIsReviewPending(true);
            result = `Changes pending review: edit_multiple_files affecting ${changes.length} file(s)`;
          }
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 custom-scrollbar">
              ...
              <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground gap-4 animate-in fade-in duration-700">
                ...
                <div className="max-w-[80%]">
                  ...
                  <p className="text-xs mt-2 leading-relaxed opacity-80">
                    I can read your configuration, scaffold new components, or
            ...
            {messages.map((msg) => {
              ...
              const toolParts: MessagePart[] = rawParts.filter((p) =>
                    ...
                    <div className="flex gap-3 mb-6">
                      ...
                      <div className="flex-1 space-y-2 min-w-0">
                        ...
                        {toolParts.map((ti) => {
                          ...
                          const tiName =
                            (ti.toolName as string | undefined) ??
                            (ti.type as string)
                              ?.split("-")
                              .slice(1)
                              .join("-") ??
                          ...
                          const tiPath = (
                            ti.input as Record<string, unknown> | undefined
                          ...
                          return (
                            <div
                              key={ti.toolCallId}
                              className="flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground border rounded-xl bg-muted/30 shadow-sm max-w-[90%]"
                            >
                              ...
                              <span className="font-mono truncate tracking-tight">
                                {tiName}({tiPath ? tiPath.split("/").pop() : ""}
                                ){" "}
                                {tiDone ? (
                                  "✓"
                                ) : (
          ...
          <div className="border-t bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 p-3 sticky bottom-0 z-10">
            ...
            <div
              className="mt-3 flex items-center justify-between px-1 relative"
              ref={pickerRef}
            >
              <button
                type="button"
              ...
              <div className="flex items-center gap-2">
                {isReviewPending && pendingChanges && (
                  <div className="text-[10px] font-medium text-amber-600 bg-amber-50 border border-amber-200 rounded-full px-2 py-0.5">
                    {pendingChanges.changes.length} AI change
                    {pendingChanges.changes.length !== 1 ? "s" : ""} pending
                    review
                  </div>
                )}
                ...
                <div className="absolute bottom-full left-0 mb-2 bg-background border rounded-xl shadow-lg shadow-black/5 p-1 min-w-[140px] z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
                  {PROVIDERS.map((p) => (
                    ...
                    >
                      <span
                        className={
                          provider === p.id ? "opacity-100" : "opacity-60"
                        }
                      >
                        {p.icon}
                      </span>
