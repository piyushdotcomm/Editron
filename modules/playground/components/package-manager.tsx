...
export function PackageManager({
  ...
  const searchNpm = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${NPM_REGISTRY_SEARCH_URL}?text=${encodeURIComponent(
          searchQuery
        )}&size=10`
      );
  ...
  const executeCommand = async (
    ...
    try {
      ...
      if (exitCode === 0) {
        toast.success(
          `Successfully ran: ${command} ${args.join(" ")}`
        );
      } else {
          ...
          <div className="space-y-2 border-b pb-4">
            ...
            {searchResults.map((result) => (
              <div
                ...
                className="flex flex-col gap-1.5 p-2 rounded-md bg-muted/30 border text-xs"
              >
                ...
                <div className="flex gap-1.5 mt-1">
                  <Button
                    ...
                    onClick={() =>
                      handleInstall(result.package.name)
                    }
                    disabled={loadingPkg === result.package.name}
                  >
                    {loadingPkg === result.package.name ? (
                  ...
                  <Button
                    ...
                    className="h-6 text-[10px] px-2 w-full"
                    onClick={() =>
                      handleInstall(result.package.name, true)
                    }
                    disabled={loadingPkg === result.package.name}
                  >
                    Add -D
            ...
            <div>
              ...
              {Object.entries(dependencies).map(
                ([name, version]) => (
                  <div
                    key={name}
                    className="group flex items-center justify-between px-2 py-1.5 rounded-md hover:bg-muted/50 cursor-pointer text-xs"
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        variant="secondary"
                        className="text-[9px] px-1 py-0 h-4 font-mono font-normal"
                      >
                        {version}
                      </Badge>
